...
class BodyFrame
{
    ...
    public BodyFrame MoveTo(BodyFrame next, float delta, Vector2[] tremble, float slug_k, float sway_k)
    {
        BodyFrame mid = new BodyFrame(count);
        for (int i = 0; i < count; i++)
        {
            mid.positions[i] = positions[i] * (1f - delta) + (next.positions[i] + tremble[i]) * delta;
            mid.rotations[i] = rotations[i] * (1f - delta) + next.rotations[i] * delta;
            mid.positions[i] *= slug_k;
            mid.rotations[i] += sway_k;
        }
        return mid;
    }
    ...
    public BodyFrame GetCopy()
    {
        BodyFrame copy = new BodyFrame(count);
        for (int i = 0; i < count; i++)
        {
