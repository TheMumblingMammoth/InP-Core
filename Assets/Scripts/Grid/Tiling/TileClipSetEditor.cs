...
using System.IO;
[CustomEditor(typeof(TileClipSet))]
public class TileClipSetEditor : Editor {
	
	public override void OnInspectorGUI(){
		TileClipSet clipSet = (TileClipSet)target;
		...
		if(GUILayout.Button("AddClip")){
			...
			if(path != null)
				if(path != ""){
					...
					clipSet.AddClip( Resources.Load<TileClip>(path));
				}
		}
	...
	void CreateClip(string path, string file_name){
		TileClipSet clipSet = (TileClipSet)target;
		TileClip temp = TileClipEditor.Load(path + "/" + file_name, Instantiate(Resources.Load<TileClip>("TileClips/NullClip"))); 
		bool success;
		path = path.Replace("Sprites", "Resources/TileClips");
		TileClip clip = PrefabUtility.SaveAsPrefabAsset(temp.gameObject, path + "/" + temp.name+".prefab" , out success).GetComponent<TileClip>();
		if(success)
	...
	void LoadClip(string path, string file_name){
		...
		if(!file_name.EndsWith("ClipSet")){
			...
			TileClip clip = Resources.Load<TileClip>(path+"/"+temp);
			clipSet.AddClip(clip);
		}
	}
